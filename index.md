...
I will give the difference as a one liner rather than mathematical expression. Monte Carlo algorithm calculates the values for a state by keeping in mind the end reward, so in our example 100 is reward, so it will calculate values for all the states leading to this end reward, where as in case of temporal learning the value is calculated for the immediate reward and not the Final reward.
